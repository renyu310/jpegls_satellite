...
static BinaryInputStream & readBit(BinaryInputStream &in, Uint32 &bit)
{
...
static BinaryOutputStream & writeBit(BinaryOutputStream &out, Uint32 bit)
{
...
static Uint32 readRow(BinaryInputStream &in, Uint16 *buffer, Uint32 n, Uint16 bpp)
{
	Uint32 count = 0;
	if (bpp <= 8) {
		while (n-- && in) {
...
static BinaryOutputStream & writeRow(BinaryOutputStream &out, Uint16 *buffer, Uint32 n, Uint16 bpp)
{
	if (bpp <= 8) {
...
static BinaryOutputStream & writeBitFlush(BinaryOutputStream &out)
{
	Assert(writeBitCount < 8);
...
static BinaryOutputStream & write16BE(BinaryOutputStream &out, Uint16 word)
{
	unsigned char byte;
	byte = word >> 8;
...
static BinaryOutputStream & write8(BinaryOutputStream &out, unsigned char byte)
{
	out.write((char *)&byte, 1);
	return out;
}

static BinaryInputStream & read16BE(BinaryInputStream &in, Uint16 &value)
{
	unsigned char byte;
	in.read((char *)&byte, 1);
	value = Uint16(byte) << 8;
...
static BinaryInputStream & read8(BinaryInputStream &in, unsigned char &byte)
{
	in.read((char *)&byte, 1);
	return in;
}
...
static BinaryOutputStream & writeSOI(BinaryOutputStream &out)
{
...
static bool readSOF55(BinaryInputStream &in, Uint16 marker, Uint16 &P, Uint32 &ROWS, Uint32 &COLUMNS)
{
	//cerr << "readSOF55:" << endl;
	Uint16 length;
...
static bool readSOS(BinaryInputStream &in, Uint16 marker, Uint16 &NEAR)
{
	Uint16 length;
	unsigned char ncomponents;
	unsigned char componentid;
...
static bool readLSE1(BinaryInputStream &in, Uint16 marker, Uint32 &MAXVAL, Uint16 &T1, Uint16 &T2, Uint16 &T3, Uint16 &RESET)
{
	Uint16 length;
	unsigned char id;
	Uint16 maxval;
...
static void quantizeErrval(Uint16 NEAR, Int32 &Errval)
{
...
static void deQuantizeErrval(Uint16 NEAR, Int32 &Errval)
{
...
bool encoder(Options opt)
{
	...
	Uint32 rows = 0;
	Uint32 cols = 0;
	Uint16 bits = 0;		// Sample precision
	Uint32 maxval = 0;
	...
	bool havaRun = opt.haveRuns();


	rows = opt.getRows();
	cols = opt.getCols();
	bits = opt.getBits();
	maxval = opt.getMaxval();
	near = opt.getNear();
	BASIC_T1 = opt.getBT1();
	BASIC_T2 = opt.getBT2();
	BASIC_T3 = opt.getBT3();
...
#define CLAMP_3(i)	(((i) > MAXVAL || (i) < T2) ? T2 : (i))
	...
	if (verbose) cerr << "COLUMNS = " << cols << endl;
	if (verbose) cerr << "P = " << bits << endl;
	...
	for (row = 0; row < ROWS; row++)
	{
		...
		for (col = 0; col < COLUMNS; col++)
		{
			...
			if (Abs(D1) <= NEAR && Abs(D2) <= NEAR && Abs(D3) <= NEAR && havaRun)
			{
				...
				if (col < COLUMNS)
				{	// Must have been terminated by different value
					//cerr << "pixel[" << row << "," << col << "] = " << thisRow[col] << endl;
					//cerr << "\tcol " << col << endl;
					...
					codecRunEndSample(thisRow[col], Ra, Rb, RANGE, NEAR, MAXVAL, RESET, LIMIT, qbpp, J[RUNIndex], A, N, Nn, in, out, false);
...
Options::Options(const string optionFile)
{
	...
	while (!confile.eof())
	{
		...
		else
		{
			...
			{
				tag = buf.substr(0, index1);
				value = buf.substr(index1 + 1);
			}
		...
		if (tag == "NEAR")
			NEAR = atoi(value.c_str());
		else if (tag == "RESET")
			RESET = atoi(value.c_str());
		else if (tag == "MAXVAL")
			MAXVAL = atoi(value.c_str());
		else if (tag == "BITS")
			BITS = atoi(value.c_str());
		else if (tag == "BASIC_T1")
			BASIC_T1 = atoi(value.c_str());
		else if (tag == "BASIC_T2")
			BASIC_T2 = atoi(value.c_str());
		else if (tag == "BASIC_T3")
			BASIC_T3 = atoi(value.c_str());
		else if (tag == "ROWS")
			ROWS = atoi(value.c_str());
		else if (tag == "COLOMNS")
			COLOMNS = atoi(value.c_str());
		else if (tag == "ENDIAN"){
			value = lownTouper(value);
			if (value == "BIG")
				ENDIAN = BigEndian;
			else
				ENDIAN = LittleEndian;
		...
		{
			cout << "\"" << tag << "\" is not a valid parameter!" << endl;
			continue;
		}
...
void Options::showConf()
{
	cout << "BASIC_T1:" << BASIC_T1 << endl;
	cout << "BASIC_T2:" << BASIC_T2 << endl;
	cout << "BASIC_T3:" << BASIC_T3 << endl;
	cout << "NEAR	:" << NEAR << endl;
	cout << "RESET	:" << RESET << endl;
	cout << "ROWS	:" << ROWS << endl;
	cout << "COLOMNS:" << COLOMNS << endl;
	cout << "MAXVAL	:" << MAXVAL << endl;
	cout << "BITS	:" << BITS << endl;
	cout << "NOMARKER:" << NOMARKER << endl;
	cout << "NORUNS	:" << NORUNS << endl;
	cout << "VERBOSE:" << VERBOSE << endl;


	cout << "ENDIAN	:" << ENDIAN << endl;
	cout << "inFile	:" << inFile << endl;
	cout << "ouFile	:" << ouFile << endl;
